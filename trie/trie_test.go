...
func TestTrie_Operation(t *testing.T) {
	...
	leaf1IR, _ := rlp.EncodeToBytes(leaf1)
	// leaf1IR, _ := proto.Marshal(&triepb.Node{Val: leaf1})
	leaf1H := crypto.Sha3b256(leaf1IR)
	...
	ext5IR, _ := rlp.EncodeToBytes(ext5)
	// ext5IR, _ := proto.Marshal(&triepb.Node{Val: ext5})
	ext5H := crypto.Sha3b256(ext5IR)
...
func TestHashIfValueChangedWithSamekey(t *testing.T) {
	...
	addr1 := common.FromHex("1f345678e9")
	val1 := []byte("value1")
	_, err := tr.Put(addr1, val1)
	if err != nil {
		fmt.Printf("%v\n", err)
	}
	addr2 := common.FromHex("1f245678e9")
	val2 := []byte("value2")
	_, err = tr.Put(addr2, val2)
	if err != nil {
		fmt.Printf("%v\n", err)
	}
